...
def confirm_download(filename: str, size_bytes: int, verbose: bool = True) -> bool:
    ...
    if size_bytes > 1024 * 1024 * 1024:  # > 1 GB
        print(
            "    WARNING: This is a large file that may take significant time to download.")
...
def run_external_command(
    ...
    Args:
        ...
        stream_output: If True, stream output to terminal in real-time (for interactive tools)
        env: Optional custom environment variables (if None, inherits current environment)
    ...
    try:
        if stream_output:
            # For interactive/long-running commands, stream output to terminal
            ...
            # For quick commands, capture output for processing
            result = subprocess.run(
                command,
                capture_output=True,
                text=True,
        ...
        if verbose:
            ...
            print(
                f"    [ERROR] Command '{cmd_name}' not found. Please install it first.")
        return False
