    ...
    def build_from_file(self, yaml_file: Path) -> Cohort:
        ...
        class ConfiguredCohort(Cohort):
            ...
            def download(self, output_dir=None, download_all=True, download_images=False, extract=True, verbose=True, confirm=True):
                ...
                if verbose:
                    print(
                        f"Downloading {cohort_info['code']} cohort to {output_path}")
                ...
                if download_all:
                    ...
                    for dataset in self.datasets:
                        ...
                        if size:
                            total_size += size
                        file_details.append(
                            (dataset.filename, size if size else 0))
                    ...
                    for dataset in self.datasets:
                        try:
                            dataset.download(str(output_path), download_images=download_images,
                                             extract=extract, verbose=verbose, confirm=False)
                        except Exception as e:
                            if verbose:
                                print(
                                    f"    [ERROR] Error downloading {dataset.name}: {e}")
