...
class ClinicalDataModule(pl.LightningDataModule):
    ...
    pathologic stage to integer labels.  The resulting batch key is
    ``"clinical"``, matching the ``ClinicalMLPEncoder`` registered under the
    same name.
    """
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        self._full_dataset = TabularDataset(df, label_col=label_col, batch_key="clinical")
        ...
        self.train_dataset = TabularDataset(
            shuffled.iloc[:train_size], label_col=label_col, batch_key="clinical"
        )
